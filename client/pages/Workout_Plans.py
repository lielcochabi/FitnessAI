...
def fetch_workouts(api_url: str, username: str):
    resp = requests.get(
        f"{api_url}/workouts",
...
# ------------------ Delete Workout ------------------ #
def remove_workout(username, workout_name):
    try:
...
with st.spinner("Loading workout plans..."):
    try:
        workouts = fetch_workouts(API_URL, username)
    except Exception as e:
        st.error(str(e))
        workouts = []
    ...
    for workout in workouts:
        ...
        with col2:
            if st.button("delete", key=f"delete_workout_{name}"):
                message = remove_workout(username, name)
                st.success(message)
                fetch_workouts.clear()
                st.rerun()
